...
Implementasi RAG menambahkan lapisan pencarian bukti/rujukan dari knowledge base lokal. RAG tidak menggantikan IndoBERT dan tidak mengubah label classifier. Runtime saat ini menggunakan knowledge-base v1.1.1 dengan 61 dokumen; 37 tambahan lolos automated checks, yang bukan human review. Report R11 v1.0.0 tetap mengukur corpus 24 dokumen/20 query yang relevance judgment-nya disetujui owner. Report R14 terpisah menerapkan 20 query yang sama ke corpus v1.1.1 dan menghasilkan Hit Rate@3/@5 1.0, Precision@3 0.383333, Precision@5 0.25, Recall@3 0.958333, Recall@5 1.0, dan MRR 0.95. Relevance judgment tetap disetujui hanya terhadap corpus v1.0.0; dokumen tambahan belum dinilai independen. Evaluasi v1.1.1 terbatas dan tidak universal. Kandidat threshold hanya eksploratif dan `RAG_MIN_SCORE` production tidak ditetapkan.
...
RAG memiliki evaluation dataset terpisah dan berversi di `datasets/rag/evaluation-v1/`. Evaluator memakai corpus version/checksum yang tercatat pada manifest run dan menjalankan `CosineIndex` serta embedding revision yang sama dengan `rag-service`. Metric calculator mendukung Hit Rate@3, Hit Rate@5, Precision@3, Precision@5, Recall@3, Recall@5, dan MRR pada tingkat document ID. Hasil reproducible untuk dataset, corpus, model revision, dan code yang sama.

Evaluation v1.0.0 berisi 20 query dengan relevance judgment yang disetujui owner untuk corpus v1.0.0 (24 dokumen). Report baseline `reports/rag-retrieval-evaluation-v1.json` melaporkan Hit Rate@3/@5 1.0, Precision@3 0.4, Precision@5 0.25, Recall@3 0.975, Recall@5 1.0, dan MRR 0.95. R14 menghasilkan artifact terpisah `reports/rag-retrieval-evaluation-v1.1.1.json` untuk corpus v1.1.1 (61 dokumen), memakai query dan relevance judgment yang sama tanpa mengubah isinya. Metric v1.1.1 adalah Hit Rate@3/@5 1.0, Precision@3 0.383333, Precision@5 0.25, Recall@3 0.958333, Recall@5 1.0, dan MRR 0.95. Approval relevance judgment tetap hanya berlaku untuk corpus v1.0.0; dokumen tambahan v1.1.1 belum mendapat judgment independen. Karena itu, metric terbatas pada set 20 query dan label relevansi yang tersedia, bukan hasil universal; dokumen baru yang belum terlabel dapat memengaruhi precision. Perbedaan angka antarcorpus bersifat deskriptif dan bukan bukti peningkatan atau penurunan kualitas absolut.

Threshold sweep R14 bersifat eksploratif. Pada min_score 0.4, coverage 100% dan Precision@5 0.279167; pada 0.5, coverage tetap 100%, Precision@5 0.490833 dan Hit Rate@5/Recall@5 0.95; pada 0.6, coverage 95%, Precision@5 0.731667 dan Recall@5 0.925. `RAG_MIN_SCORE` tidak diubah dan threshold production belum ditetapkan. Threshold similarity berbeda dari confidence threshold classifier.
...
Production KB memiliki schema, manifest, checksum, dan validator terpisah. Snapshot runtime v1.1.1 memuat 61 dokumen: 24 record legacy yang sebelumnya ditinjau dan 37 tambahan yang lolos automated source/content checks. Sebanyak 21 kandidat dikecualikan dan 16 masih memerlukan review; tidak ada klaim bahwa 37 hasil otomatis telah human-reviewed. Report R11 baseline tetap mengikat corpus v1.0.0 berisi 24 dokumen; report R14 terpisah mengevaluasi corpus v1.1.1 dengan 20 query lama dan menjelaskan batas judgment yang belum mencakup dokumen tambahan. Service mendukung kondisi KB kosong, retrieval kosong tidak menghasilkan evidence palsu, dan kegagalan RAG tidak menghapus hasil classifier.
...
## 24.3 R14 - evaluasi terbatas pada corpus v1.1.1

R14 menjalankan ulang evaluator R11 dengan 20 query v1.0.0 pada corpus runtime v1.1.1 berisi 61 dokumen. Hasil dan failure analysis tersimpan di `reports/rag-retrieval-evaluation-v1.1.1.json`; report baseline v1.0.0 tidak ditimpa. Relevance judgment disetujui owner untuk corpus v1.0.0 saja. Dokumen tambahan belum memiliki judgment independen, sehingga hasil ini terbatas dan tidak universal; metrik precision dapat dipengaruhi oleh dokumen relevan yang belum dinilai. Threshold sweep hanya eksplorasi dan `RAG_MIN_SCORE` belum ditetapkan.

## 24.4 R13 dan pekerjaan lanjutan

R13 memperluas corpus melalui pemeriksaan otomatis atas provenance dan isi. Sebanyak 37 kandidat baru masuk snapshot runtime setelah melewati seluruh gate otomatis; 21 dikecualikan dan 16 tetap review-required. Pemeriksaan ini bukan human review dan bukan evaluasi retrieval formal. Evaluasi pada R14 menggunakan query lama, sehingga evaluasi yang lebih representatif memerlukan query dan relevance judgment independen yang mencakup topik serta dokumen baru. Jangan menetapkan `RAG_MIN_SCORE` tanpa dasar yang cukup. Questionnaire/usability testing untuk menilai kegunaan referensi masih merupakan pekerjaan lanjutan.
