...
**Reference:** `PROJECT_PROGRESS.md` (re-audit 20 August 2026; this checklist re-audited 21 August 2026; C4–C16, C10–C14, C18 re-audited 21 August 2026), `PRD-Sistem-Deteksi-Hoax-BERT.md`
**Overall completion: 98%** — pipeline queue chain connected (C1, C6 done) and failure-safe (C7 done); research dataset prepared (C2 done); IndoBERT fine-tuned and versioned (C3 done, with style-bias caveat); FastAPI wired with verified artifact and threshold handling (C4 done) and metrics (C18 done); Laravel↔FastAPI proven via contract + HTTP-fake + real queue inference (C5 done); ownership and UI correctness fixed (C8–C9 done); test suite runnable without MySQL and version control restored (C15–C16 done); video URL, article, media, and OpenAI hardening completed (C10–C14 done).
...
**Re-audit 21 August 2026 (C4–C16, C10–C14, C18):** C2/C3 completed (7816 rows, 498 MB artifact); C4 wired (`bert-service/.env` with `BERT_MODEL_PATH`, dotenv `app/config.py:13`, label-map verified `app/inference.py:111`, sidecars + checksum, `GET /health/ready` 200); C5 proven (`test_api.py` 12 tests, `BertClassifierTest` 12 tests, `RealBertInferenceTest` 4 tests via `queue:work`); C7 added `failed()` hooks, C8 ownership check + throttle, C9 fixed stale wording; C10 added `downloadVideoSafely()` with SSRF/redirect/size guards and `media:prune` retention, C11 replaced `strip_tags` with DOM readability + DNS rebinding checks, C12 added `dimensions`/`ffprobe` guards + `MalwareScanner` + `media:prune` scheduler; C13 added `OpenAiQuota` (rate-limit + monthly quota + cost ledger, `estimateCost` per model), C14 added shared `CircuitBreaker` to `OpenAiImageExtractor`/`OpenAiVideoExtractor`, C18 added `GET /metrics` + `Dockerfile` with `HEALTHCHECK`; C15 `phpunit.xml` now uses `sqlite :memory:` (46 + 64 tests pass), C16 `git init` repaired (commit 861f98d). `PROJECT_PROGRESS.md` remains outdated.
...
2. **D3/D4** — Add Livewire polling and Filament dashboard widgets for thesis demo completeness (`C18` Docker + `/metrics` done, remaining `docker-compose`/`supervisord` low priority).
