...
Environment RAG Docker di `.env.docker.example` memakai `RAG_SERVICE_URL=http://rag:8002`, connect timeout 3 detik, timeout 15 detik, `RAG_SERVICE_TOKEN` kosong, `RAG_TOP_K=3`, dan `RAG_MIN_SCORE` kosong. RAG service saat ini tidak menegakkan bearer-token auth; batas akses Docker adalah network internal dan tidak ada port host. Jangan menganggap `RAG_SERVICE_TOKEN` sebagai access control RAG atau mengisi threshold retrieval seolah-olah itu threshold classifier. Snapshot data-preparation memuat 24 dokumen; queue worker tetap memakai healthcheck liveness `/health/live`, sedangkan `/health/ready` mengukur kesiapan knowledge base dan model. Bila knowledge base kosong, readiness melaporkan `knowledge_base_empty` dengan HTTP 503 tanpa menjadikan proses container unhealthy.
